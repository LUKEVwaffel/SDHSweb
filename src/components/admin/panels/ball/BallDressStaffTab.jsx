...
// Provisions dress/attire approvers via ball-dress-set-pin. Two roles:
...
//                         (/ball/attire).
// Login is EMAIL-ONLY (no PIN, no password) — adding a row here is all the
// setup there is; the edge function also creates the sign-in account. The
// "who's set up" list comes from ball_dress_staff_status() (SECURITY DEFINER,
// is_s6 inside — run supabase/ball_dress_staff_status.sql).
    ...
    <div style={{ maxWidth: 620 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: sp[3] }}>
        <p style={{ fontFamily: mono, fontSize: 12, color: P.mute, margin: 0, maxWidth: 460 }}>
          Approvers sign in with their email only — no PIN, no password. Adding a name + email below is the whole setup.
        </p>
...
function StaffAdder({ role, onDone }) {
  ...
  async function submit() {
    ...
    const { data, error } = await SB.functions.invoke('ball-dress-set-pin', {
      body: { email: row.email.trim(), name: row.name.trim(), role },
    });
    setBusy(false);
    if (error || data?.error) { setFlash(`Failed: ${data?.error || error.message}`); return; }
    setFlash('Approver added ✓');
    setRow({ email: '', name: '' });
    onDone();
  }
  ...
  return (
    <div style={{ border: `1px solid ${P.hair}`, padding: sp[3] }}>
      <div style={{ fontFamily: mono, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: P.mute, marginBottom: sp[2] }}>
        Add / re-activate an approver
      </div>
      <div style={{ display: 'flex', gap: sp[2], alignItems: 'flex-end' }}>
        <div style={{ flex: 1 }}><Label>NAME</Label><Input value={row.name} onChange={(e) => set('name', e.target.value)} placeholder="Approver name" /></div>
