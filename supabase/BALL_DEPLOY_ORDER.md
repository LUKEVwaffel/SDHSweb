...
| `ball_guest_cash_split.sql` | `ball_guests.friend_cash_received` — separate cash toggle for a `self_pays` friend guest's own $35, distinct from the host's `cash_received`. Widens `ball_guests_ops_view`. **Must be followed by re-running `ball_guards.sql` (now v7)** — it adds the reviewer branch on `ball_guests_column_guard()` that lets ops flip this column. | `ball_guest_model.sql`, then `ball_guards.sql` v7 |
| `ball_guest_form_split.sql` | `ball_guests.field_trip_form_received` — separate field-trip-form toggle for the GUEST, distinct from the host's `field_trip_form_received` on `ball_signups`. Widens `ball_guests_ops_view`. **Must be followed by re-running `ball_guards.sql` (now v7)**. | `ball_guest_cash_split.sql`, then `ball_guards.sql` v7 |
